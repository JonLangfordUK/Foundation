...
- `2026-07-15`: Fixed second workflow failure from the Windows runner: `cargo` was not on the service PATH. Added workflow steps that prepend common Rust install paths (`%USERPROFILE%\.cargo\bin` and `C:\Users\jonla\.cargo\bin`) and verify `cargo`/`rustup` before validation and packaging.
