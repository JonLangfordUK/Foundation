...
## Crate Boundaries
- `crates/foundation` owns the Foundation engine executable. It wraps Bevy app construction, parses launch arguments such as `--game template-game` and `--editor`, installs Foundation runtime/editor plugins, and selects the registered game to run.
...
- Do not hard-code template-game as the only possible engine target except as a default/registered game entry.
